...
for (const itemProducto of articulos) {
  ...
  <img class="img-fluid" src="${itemProducto.imagen}" alt="${itemProducto.nombre}" onClick="agregarProducto(${itemProducto.id})">
   <p>${itemProducto.nombre}<br/> ${itemProducto.descripcion} <br/> Precio: ${itemProducto.precio} </p>
   <p><button class="btn btn-dark" onClick="agregarProducto (${itemProducto.id})">Agregar al Carrito</button></p>
  </div>`;
}
